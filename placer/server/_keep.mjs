...
function loadPlacedObjects() {
    try {
        ...
        if (existsSync(OBJECTS_FILE)) {
            ...
            alt.log(`[SERVER] Gefundene Objekte: ${data.length}`);
            ...
            data.forEach(obj => {
                if (obj && obj.id && obj.model && obj.position && obj.rotation) {
                    ...
                    alt.log(`[SERVER] Objekt ${obj.id} geladen: Model ${obj.model} at ${JSON.stringify(position)}`);
                }
            });

            alt.log(`[SERVER] ${placedObjects.size} Objekte erfolgreich geladen`);
        } else {
            alt.log('[SERVER] Keine placed_objects.json gefunden, starte mit leerer Map');
            placedObjects.clear();
            // Erstelle leere Datei
            writeFileSync(OBJECTS_FILE, '[]');
        }
    } catch (error) {
        ...
        try {
            if (existsSync(BACKUP_FILE)) {
                alt.log('[SERVER] Versuche Backup zu laden...');
...
alt.onClient("PlacingModule:DeleteObject", (player, data) => {
    ...
    for (let [id, objectData] of placedObjects) {
        // Vergleiche Position und Model
        const distance = calculateDistance(objectData.position, data.position);
        
        alt.log(`[SERVER] Vergleiche Objekt ${id}:`);
        alt.log(`Position Distanz: ${distance}`);
        alt.log(`Model: ${objectData.model} vs ${data.model}`);
        
        // Erhöhe die Toleranz für die Position und prüfe das Model
        if (distance < 1.0 && objectData.model === data.model) {
            alt.log(`[SERVER] Objekt gefunden! ID: ${id}`);
