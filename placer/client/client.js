    ...
    if (raycastResult.isHit && raycastResult.entityType === 3) {
        ...
        if (targetObject && game.isEntityAnObject(targetObject)) {
            ...
            if (game.isControlJustPressed(0, 73)) { // X Taste
                const modelHash = Math.abs(game.getEntityModel(targetObject));
                const pos = game.getEntityCoords(targetObject, false);
                
                alt.log('[CLIENT] Versuche Objekt zu löschen:', {
                    model: modelHash,
                    position: pos
                });
                
                // Löschanfrage an den Server senden
...
if (key === 0x45) { // E zum Platzieren
    if (showObject && newObjectToPlace) {
        ...
        // Das finale Objekt wird jetzt vom Server über PlacingModule:AddObject erstellt
        // Wir müssen es hier nicht mehr lokal erstellen, da der Server es allen Clients mitteilt

        // Nur das temporäre Objekt entfernen
        game.deleteObject(newObjectToPlace);
        newObjectToPlace = null;
        showObject = false;
        lastRotation = { x: 0, y: 0, z: 0 };
